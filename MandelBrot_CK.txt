...
public class Mandelbrot_CK
{
    ...
    private Mandelbrot_CK(int accuracy, int maxIterations, int maxSide, double startX, double endX, double startY, double endY, double size)
    {
        ...
        this.endY = endY;
        this.size = size;
        picture = new Picture(maxSide+1,maxSide+1); //I make the picture width and height one more than it should because picture objects
    }
    ...
    private void checkRegion(int i, int j, int side)
    {
        ...
        else //IF NOT BASE CASE
        {
             ...
             */
            if (allTheSame) //the points are all the same (they passed inspection)
            {
                Random rand = new Random();
    ...
    {
        Mandelbrot_CK mandel = new Mandelbrot_CK(100, 300,1024,-1.5,.5, 1.0, -1.0, 2.0);
